...
struct LogChangesReducer<Base: Reducer>: Reducer {
    ...
    func reduce(
        ...
        let box = UncheckedSendableBox(value: (action: action, oldState: oldState, newState: state, logger: logger))
        return effects.merge(
            with: .run { _ in
                box.value.logger.logChange(receivedAction: box.value.action, oldState: box.value.oldState, newState: box.value.newState)
            }
        )
    }
}
